...
void main() {  
	...
	for(int i=0;i<samplenum;i++){//64*√3より
		...
		if(src.x<threshold){
			...
			//float diff=normal.length();//最大で1かなぁ 勾配をとったがあまり効果はなかった。
			//normal=normalize(gl_NormalMatrix*normal);//
		//	normal=normalize(gl_NormalMatrix*normal);
			//float hue=src.x*360.0;
			float hue=(0.65-src.x)/0.45*240.0;//ここに何かの信号を与える
		  ...
		  vec4 c;
           ...
           case 5: c =vec4(1.0,0.0,dChanging,scalar); break;
			...
			src=vec4(shading(normal,viewvec,lightvec)*c.rgb,scalar);
		}
		//front-to-back
